...
msg "Package..."
srcdir=$(pwd)/../src
macdeployqt src/LiriCalculator.app -dmg -qmldir=$srcdir -verbose=2
mkdir -p travis-out
mv src/LiriCalculator.dmg travis-out/LiriCalculator-git-$(date +%Y%m%d-%H%M%S)-$(git rev-parse --short HEAD).dmg
travis_end "package"
